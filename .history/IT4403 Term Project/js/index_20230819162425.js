$(document).ready(function () {
    //clear session store for when users perform a reload of the page
    sessionStorage.clear();

    //save url1 & url2 to session storage - this handles when a user requests additional pages for "trending"
    ...
    //hiding buttons that control list & grid view buttons
    $("#listLayout").hide();
    $("#gridLayout").hide();
    

    // var radioValue = "";
    //url that is used for first page loads to display "trending" content
    var url = "https://api.themoviedb.org/3/trending/all/day?api_key=596a559ed3e4a3408502d51411f3d607&page=1";
    var multi_media_type = {};

    //these bool variables are used in the searchVidoesandPeople &  getVideoOrPersonDetails functions
    //flow control below that decides what the user is wanting additional
    //information on such as a clicked movie, person, cast member, etc...
    var getPerson = false;
    ...
    //click function to handle user requests for returning home
    $(".home").click(function() {
        $(".trending").show();
    ...
    ////click function to handle user requests for "multi search" content
    ...
    //click events to pages so additional ajax can be triggered
    $("#pages").on('click', function (event) {

        var number = $(event.target).text();
        url = sessionStorage.getItem("url1") + number + sessionStorage.getItem("url2");
        //console.log(url)
        // radioValue = $("input[name='user_selection']:checked").val();
        var isAdult = $("input[name='safeSearch']:checked").val();
    ...
    function searchVidoesandPeople(servicePoint) {
        //clear parent/children containers for content to be loaded on the page
        $(".cast").empty();
        $(".div11").empty();
        $(".div22").empty();
        $(".div33").empty();
        ...
        $.getJSON(servicePoint, function (jsonData) {
            ...
            //by checking if multiSearch is true or if the returned json has a media_type property
            //****NOTE THAT MAY BE ABLE TO REMOVE THE HASOWNPROPERTY SINCE ISMULTISEARCH IS HANDLING THE CHCK
