    ...
    try {
      ...
      $.getJSON(url, function (jsonData) {
        ...
        if (
          ...
          try {
            try {
                // Attempt to fetch JSON data from the specified 'url'
                $.getJSON(url, function (jsonData) {
                ...
                // Handle an error scenario when casting information cannot be retrieved
                cast = "We don't have any cast to add to this show";
                $(".div33").append(cast).css({ "margin-top": "0" });
              
                // Set the 'getPerson' flag to false
                getPerson = false;
              }
              
              // Initialize variables to store details about the media (title, overview, genres, etc.)
              var title = "";
              var overview = "";
              var genres = "";
              ...
              // Check if an overview is available and handle translations or absence
              if (
                ...
                // Use the provided overview if available
                console.log("else language called");
                overview = jsonData.overview;
              }
              
              // Determine media type and extract relevant details (title, release date, runtime, etc.)
              ...
              // Determine media title based on available properties
              ...
              // Determine media poster image path, handle absence
              ...
              // Generate genre links if genres are available, otherwise set genres to an empty string
              ...
              // Construct the HTML content for the media details section
              ...
              // Clear existing content and update the HTML with the generated 'movie' content
              $("#results").html("");
              $(".parent").html("");
