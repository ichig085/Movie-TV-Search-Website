    ...
    $.getJSON(servicePoint, function (jsonData) {
      ...
      if (isMultiSearch || jsonData.results.hasOwnProperty("media_type")) {
        ...
        if (Object.keys(multi_media_type).length) {
          Object.keys(multi_media_type).map(
            (key) => delete multi_media_type[key]
          );
        }
        for (i = 0; i < jsonData.results.length; i++) {
          vote_average = Math.round(jsonData.results[i].vote_average);
          multi_media_type[jsonData.results[i].id] =
            jsonData.results[i].media_type;

          image = `https://image.tmdb.org/t/p/w500/`;

          if (
            (jsonData.results[i].hasOwnProperty("poster_path") &&
              jsonData.results[i].poster_path == null) ||
            (jsonData.results[i].hasOwnProperty("profile_path") &&
              jsonData.results[i].profile_path == null)
          ) {
          ...
          videoOrPersonDiv +=
            `<img style='cursor:pointer' class='movieID' id='${jsonData.results[i].id}' src='` +
            image +
            "' />";
        ...
        for (i = 0; i < jsonData.results.length; i++) {
          ...
          image = `https://image.tmdb.org/t/p/w500/`;
          if (
            jsonData.results[i].hasOwnProperty("media_type") &&
            jsonData.results[i].media_type == "person"
          ) {
