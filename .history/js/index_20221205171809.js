        ...
        try {
            ...
            $.getJSON(url, function (jsonData) {
                ...
                if (sessionStorage.getItem("media_type") == "person" || sessionStorage.getItem("name") != null || multi_media_type[movieID] == "person") {
                    ...
                    if (jsonData.hasOwnProperty("genres")) {
                        ...
                        for (var i = 0; i < jsonData.genres.length; i++) {
                            if (i == jsonData.genres.length - 1) {
                                genres += `<a class="genreSearch" id="${genres[i].id}" style="cursor:pointer;">${jsonData.genres[i].name}</a>`;
                            }
                            else {
                                genres += `<a class="genreSearch" id="${genres[i].id}" style="color:blue; cursor:pointer;">${jsonData.genres[i].name}</a>, `;
                            }
                        }
                    }
                    else {
                    ...
                    movie += "</div>"
                    $(".genreSearch").on('click', function () { console.log(getVideoOrPersonDetails($(this).attr("id"))); });
                    $("#results").html("");
                    $(".parent").html("");
