        ...
        try {
            ...
            $.getJSON(url, function (jsonData) {
                ...
                if (sessionStorage.getItem("media_type") == "person" || sessionStorage.getItem("name") != null || multi_media_type[movieID] == "person") {
                    ...
                    if (jsonData.hasOwnProperty("also_known_as")) {
                        if (jsonData.also_known_as == "" || jsonData.also_known_as == null) {
                            ...
                            for (var i = 0; i < jsonData.also_known_as.length; i++) {
                                also_known_as += `<p style='margin-top:.25rem; font-size: 1.025rem;'>${jsonData.also_known_as[i]}</p>`
                            }
                        }
                    }
                    else {
                        also_known_as += `<p style='margin-top:.25rem; font-size: 1.025rem;'>No information found.</p>`
                    }
                    if (gender == "1") {
                        gender = "Female";
                    }
                    ...
                    movie = `<div class='div11'><img src='${image}' style='width: 100%; height: auto;'/><h3>Personal Info</h3><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Known For</p><p style='margin-top:.25rem; font-size: 1.025rem;'>${known_for}</p><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Gender</p><p style='margin-top:.25rem; font-size: 1.025rem;'>${gender}</p><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Birthday</p><p style='margin-top:.25rem; font-size: 1.025rem;'>${birthday}</p><p class='personalInfo' style='font-weight: bold; margin-bottom:0;'>Place of Birth</p><p style='margin-top:.25rem; font-size: 1.025rem;'>${place_of_birth}</p><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Also Known As</p>${also_known_as}</div>`
                    movie += `<div class='div22' style='width:80%;'><h2 style='padding-left: 1rem; margin-top: 0;'>${name}</h2><h3 style='padding-left: 1rem; margin-bottom: .5rem;'>Biography</h3><p style='font-size: 1.025rem; padding-left: 1rem; margin-top: 0;'>${biography}</p></div>`
                    ...
                    movie += `<p style='font-size:1.025rem; margin-top: .25rem;'>${overview}</p>`;
