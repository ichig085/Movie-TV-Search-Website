        ...
        $.getJSON(servicePoint, function (jsonData) {
            ...
            if (isMultiSearch || jsonData.results.hasOwnProperty("media_type")) {
                ...
                for (i = 0; i < jsonData.results.length; i++) {
                    vote_average = Math.round(jsonData.results[i].vote_average);
                    ...
                    videoOrPersonDiv = "<div class='vidOrPersonContainer'>";
                    videoOrPersonDiv += `<img style='cursor:pointer' class='movieID' id='${jsonData.results[i].id}' src='` + image + "' />";
                    videoOrPersonDiv += "<h3 style='padding:3%;'>" + title + "</h3>";
                    videoOrPersonDiv += `<p style="padding: 3%; margin-top: 1rem;">Rating: ${vote_average}</p></div>`;
                ...
                for (i = 0; i < jsonData.results.length; i++) {
                    ...
                    videoOrPersonDiv = "<div class='vidOrPersonContainer'>";
                    videoOrPersonDiv += `<img style='cursor:pointer' class='movieID' id='${jsonData.results[i].id}' src='` + image + "' />";
                    videoOrPersonDiv += "<h3 style='padding:3%; margin-bottom: 0;'>" + title + "</h3>";
                    videoOrPersonDiv += `<p style="padding: 3%; margin-top: 1rem;">Rating: ${vote_average}</p></div>`;
        ...
        try {
            ...
            $.getJSON(url, function (jsonData) {
                ...
                if (sessionStorage.getItem("media_type") == "person" || sessionStorage.getItem("name") != null || multi_media_type[movieID] == "person") {
                    ...
                    if (jsonData.hasOwnProperty("biography")) {
                        if (jsonData.biography == "") {
                            ...
                            for (var i = 0; i < jsonData.biography.length; i++) {
                                ...
                                // console.log(isChar)
                                if (jsonData.biography.charAt(i) == "." && isCharBefore == true && isCharAfter == true) {
                                    i++
                                    continue;
                                }
                                else {
                    ...
                    if (jsonData.hasOwnProperty("also_known_as")) {
                        if (jsonData.also_known_as == "" || jsonData.also_known_as == null) {
                            ...
                            for (var i = 0; i < jsonData.also_known_as.length; i++) {
                                also_known_as += `<p style='margin-top:.25rem; font-size: 1.1rem;'>${jsonData.also_known_as[i]}</p>`
                            }
                        }
                    }
                    else {
                        also_known_as += `<p style='margin-top:.25rem; font-size: 1.1rem;'>No information found.</p>`
                    }
                    if (gender == "1") {
                        gender = "Female";
                    }
                    ...
                    movie = `<div class='div11'><img src='${image}' style='width: 100%; height: auto;'/><h2>Personal Info</h2><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Known For</p><p style='margin-top:.25rem; font-size: 1.1rem;'>${known_for}</p><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Gender</p><p style='margin-top:.25rem; font-size: 1.1rem;'>${gender}</p><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Birthday</p><p style='margin-top:.25rem; font-size: 1.1rem;'>${birthday}</p><p class='personalInfo' style='font-weight: bold; margin-bottom:0;'>Place of Birth</p><p style='margin-top:.25rem; font-size: 1.1rem;'>${place_of_birth}</p><p class='personalInfo' style='font-weight:bold; margin-bottom:0;'>Also Known As</p>${also_known_as}</div>`
                    movie += `<div class='div22' style='width:80%;'><h1 style='padding-left: 1rem; margin-top: 0;'>${name}</h1><h2 style='padding-left: 1rem; margin-bottom: .5rem;'>Biography</h2><p style='font-size: 1.1rem; padding-left: 1rem; margin-top: 0;'>${biography}</p></div>`
                    ...
                    movie += `<div class='div22' style='margin-left: 1.85rem; margin-bottom:0; width: 50%'><h1 style='margin-bottom:0;'>${title}</h1>`;
                    movie += `<p style='margin-top: .25rem; font-size: .95rem;'>${release_date} ${genres} ${runtimeHours} ${runtimeMinutes}</p>`;
                    movie += "<h2 style='margin-bottom: 0;'>Overview</h2>";
                    movie += `<p style='font-size:1.1rem; margin-top: .25rem;'>${overview}</p>`;
